...
function SceneViewModel(tank) {
  ...
  self.maxId = ko.pureComputed(function () {
    var id = 0;
    var scenes = tank.scenes();
  ...
  self.save = function () {
    tank.scenes.push(new Scene({
      id: parseInt(self.id()),
      name: self.name(),
      fadeIn: parseInt(self.fadeIn()),
      fadeOut: parseInt(self.fadeOut()),
      ch1: self.ch1(),
      ch2: self.ch2(),
      ch3: self.ch3()
    }));
...
function EnviromentViewModel(app) {
  ...
  self.temp = ko.observable(28.4);
}

function TankViewModel(app) {
  var self = this;

  self.scene = ko.observable(new SceneViewModel(self));
